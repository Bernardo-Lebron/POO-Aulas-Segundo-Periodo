...
public enum Tipo {
    FOGO, AGUA, PLANTA, ELETRICO, NORMAL, VENENO, FANTASMA;
    ...
    public static double obterMultiplicador(Tipo tipoAtaque, Tipo tipoDefensor) {
        ...
        return switch (tipoAtaque) {
            ...
            case ELETRICO -> switch (tipoDefensor) {
                ...
                case PLANTA, ELETRICO -> 0.5; // Planta resiste Eletrico
                default -> 1.0;
            };
            case VENENO -> switch (tipoDefensor) {
                case PLANTA -> 2.0; // Veneno envenena Planta
                case VENENO -> 0.5; // Veneno resiste Veneno
                default -> 1.0;
            };
            case FANTASMA -> switch (tipoDefensor) {
                case NORMAL -> 0.0; // Normal é imune a Fantasma
                case FANTASMA -> 2.0; // Fantasma é super efetivo contra Fantasma 
                default -> 1.0;
            };
